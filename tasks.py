...
def eval(ctx: Context, model_checkpoint: str = "models/model.pth") -> None:
    """Evaluate the model."""
    ctx.run(
        f"uv run python -c \"from src.{PROJECT_NAME}.evaluate import evaluate; evaluate('{model_checkpoint}')\"",
        echo=True,
        pty=not WINDOWS,
    )
...
def visualize(ctx: Context, model_checkpoint: str = "models/model.pth", figure_name: str = "embeddings.png") -> None:
    ...
    ctx.run(
        f"uv run python -c \"from src.{PROJECT_NAME}.visualize import visualize; visualize('{model_checkpoint}', '{figure_name}')\"",
        echo=True,
        pty=not WINDOWS,
    )
